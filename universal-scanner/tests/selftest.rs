...
/// 期望元组 (protocol, version, ip, mac, device_type, serial)
type Tuple = (String, u32, String, String, String, String);
/// 期望表键 (engine_name, fixture)
type Key = (String, String);

fn expected_tuples() -> BTreeMap<Key, Vec<Tuple>> {
    let mut m = BTreeMap::new();
    // (protocol, version, ip, mac, device_type, serial)
    m.insert(
    ...
    m.insert(
        ...
        vec![(
            ...
            1,
            "240.0.16.0".into(),
            String::new(), // "MAC" TXT 变量是设备 ID 串，非 MAC 地址
            "AV1000-VIRTUAL   ".into(),
            "001A07334455".into(),
        )],
    );
    ...
    for (re, devs) in &all {
        ...
        for (i, (d, e)) in devs.iter().zip(exp.iter()).enumerate() {
            assert_eq!(
                ...
                (e.0.as_str(), e.1, e.3.as_str(), e.4.as_str(), e.5.as_str()),
                "{}[{}]: (protocol,version,mac,type,serial) 元组不符",
                re.fixture,
                i
            );
            assert_eq!(
